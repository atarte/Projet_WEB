...
Class Offre_Model extends Model {
    ...
    public function deleteWish(int $id) {
        $req = "SELECT Candidature.Id_Stage AS id_stage, Candidature.Id_Users AS id_user FROM Candidature WHERE Candidature.Id_Users = 5 AND Candidature.Id_Stage = 1 AND (Candidature.Postulation IS NOT NULL)";
        ...
        if ($count == 0) {
            $req = "CALL Delete_Wishlist(:id_user, :id_offre)";
        }
        else {
            $req = 'UPDATE Candidature SET
